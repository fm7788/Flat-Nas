...
func GetRssMeta(c *gin.Context) {
	...
	title, err := fetchRssTitle(urlStr)
	if err != nil {
		c.JSON(http.StatusInternalServerError, map[string]interface{}{"success": false, "error": err.Error()})
		return
	}
...
func fetchRssTitle(feedUrl string) (string, error) {
	feedUrl = strings.TrimSpace(feedUrl)
	if feedUrl == "" {
	...
	var lastErr error
	for _, candidate := range candidates {
		title, err := fetchRssTitleOnce(candidate)
	...
	return "", fmt.Errorf("failed to parse feed title")
}
...
func parseRssTitle(body []byte) (string, error) {
	var rss2 Rss2Feed
	decoder := xml.NewDecoder(bytes.NewReader(body))
	decoder.CharsetReader = charset.NewReaderLabel
	if err := decoder.Decode(&rss2); err == nil && strings.TrimSpace(rss2.Channel.Title) != "" {
		return strings.TrimSpace(rss2.Channel.Title), nil
	}

	var atom AtomFeed
	decoder = xml.NewDecoder(bytes.NewReader(body))
	decoder.CharsetReader = charset.NewReaderLabel
	if err := decoder.Decode(&atom); err == nil && strings.TrimSpace(atom.Title) != "" {
	...
	decoder = xml.NewDecoder(bytes.NewReader(body))
	decoder.CharsetReader = charset.NewReaderLabel
	if err := decoder.Decode(&rdf); err == nil && strings.TrimSpace(rdf.Title) != "" {
		return strings.TrimSpace(rdf.Title), nil
	}
